...
function ModalDetalhesEvento({ evento, onClose }) {
  ...
  return (
    <div className="detalhes-evento-modal-overlay">
      <div className="detalhes-evento-modal">
            ...
            <div className="item" key={i} onClick={() => evento && onEventoClick(evento)} style={{ cursor: 'pointer' }}>
              {evento ? (
                <>
                  ...
                  <p className="evento-data"><strong>Início:</strong> {evento.dataInicio}</p>
                  <p className="evento-data"><strong>Fim:</strong> {evento.dataFim}</p>
                </>
              ) : null}
            </div>
          );
        ...
        <div className="grid-container extras">
          {eventosExtras.map((evento, index) => (
            <div className="item" key={index + 5} onClick={() => onEventoClick(evento)} style={{ cursor: 'pointer' }}>
              <h4>{evento.titulo}</h4>
              <p><strong>Início:</strong> {evento.dataInicio}</p>
              <p><strong>Fim:</strong> {evento.dataFim}</p>
            </div>
          ))}
        </div>
      )}
